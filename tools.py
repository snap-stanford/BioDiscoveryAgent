...
class GenePerturbAgent(object):
    ...
    def load_datasets(self):
        ...
        if not self.args.combinatorial:
            ...
            ground_truth['Gene_pairs'] = ground_truth['Gene_pairs'].apply(
                ast.literal_eval)
    ...
    def generate_prompt(self, gene_readout=None, hits=None):
        ...
        prompt += self.current_history["initial_prompt"].format(
    ...
    def append_experiment_results(self, gene_readout, hits):
        if len(gene_readout) < 1500:
            ...
            negative_summary = self.summarize_results(gene_readout.drop(hits),
                                                      "negative")
            positive_summary = self.summarize_results(
                self.ground_truth.loc[hits], "positive")
            return (
    ...
    def summarize_results(self, data, result_type):
        ...
        log_file = os.path.join(self.log_dir,
                                f"step_{self.curr_step}_log_{result_type}_sum.log")
        return complete_text(summary_prompt, model="claude-1",
                             log_file=log_file)
    ...
    def save_sampled_genes(self, gene_sampled, curr_sample):
        ...
        np.save(os.path.join(self.log_dir,
                             f'sampled_genes_{self.curr_step + 1}.npy'),
                all_sampled_so_far)
    ...
    def process_completion(self, prompt, gene_sampled, curr_sample, log_file):
        ...
        for itr in range(self.args.prompt_tries):
            if self.use_gpt4:
                completion = complete_text_gpt4(prompt_try,
                                                stop_sequences=["Observation:"],
                                                log_file=log_file)
            else:
                completion = complete_text(prompt_try, model=self.args.model,
                                           log_file=log_file)
                if "Gene Search:" in completion:
                    completion_pre = completion.split("4. Solution:")[0]
                    completion_pre += "Gene Search Result:"+self.gene_search_f(
                        completion_pre.split("Gene Search:")[1].strip(),
                        self.args.gene_search_diverse)
            ...
            try:
                entries = self.parse_entries(completion, [e.strip() for e in
                                                     self.valid_format_entries])
                valid_format = True
            except:
                valid_format = False
            ...
            if not valid_format:
                ...
                pred_genes = [p.strip(' \n[]') for p in
                              entries['Solution'].replace("\n", ",").split(',')]
                pred_genes = [p.split('.')[-1].strip(' ') for p in pred_genes]
                if self.args.combinatorial:
                    pred_genes = [tuple(sorted(s.split(" + "))) for s in
                                  pred_genes]

                gene_next_sample = list(
                    set(pred_genes).intersection(set(self.measured_genes)))
                dropped_genes = set(pred_genes) - set(gene_next_sample)
                curr_sample, prompt_update = self.process_valid_output(
                    gene_next_sample, curr_sample, gene_sampled, dropped_genes,
                    self.args)
                ...
                if itr >= self.args.prompt_tries - 4:
                    if genes_remain_summary is None:
                        ...
                        genes_remain = list(set(self.measured_genes).difference(
                            set(gene_sampled)))
                        genes_remain_summary = self.summarize_remaining_genes(
                            genes_remain)
                    else:
                        genes_remain_summary = list(
                            set(genes_remain_summary).difference(
                                set(curr_sample)))
                    prompt_update = self.gene_choices_prompt(prompt_update,
                                                        num_genes_pick,
                                                        genes_remain_summary)
    ...
    def run_experiment(self):
        ...
        for self.curr_step in range(self.steps):
            if self.curr_step != 0:
                ...
                hits = list(
                    set(self.gene_sampled).intersection(self.all_hit_genes))
                print(self.curr_step, 'Number of cumulative hits:',
                      str(len(hits)))

            prompt = self.generate_prompt(
                gene_readout if self.curr_step != 0 else None,
                hits if self.curr_step != 0 else None)
            log_file = os.path.join(self.log_dir,
                                    f"step_{self.curr_step}_log.log")
            curr_sample = []
...
"""
    ...
    def gene_search_f(self, gene_name, gene_search_diverse, **kwargs):
        print(gene_name)
        df = pd.read_csv(
            "/dfs/user/yhr/AI_RA/research_assistant/datasets/features/achilles.csv")
        ...
        if gene_search_diverse:
            return ", ".join((df[gene_name].dot(df) / (
                        np.linalg.norm(df, axis=0) * np.linalg.norm(
                    df[gene_name]))).sort_values(ascending=True)[
                             :50].index.tolist())
        else:
            return ", ".join((df[gene_name].dot(df) / (
                        np.linalg.norm(df, axis=0) * np.linalg.norm(
                    df[gene_name]))).sort_values(ascending=False)[
                             :11].index.tolist()[1:])

    def parse_entries(self, s, entries):
        pattern = ""
        for e in entries:
            e = e.replace("[", "\[").replace("]", "\]")
            pattern += f"{e}:([\s\S]*)"
    ...
    def summarize_remaining_genes(self, all_genes, summary_size=20, bs=1000):
    ...
    def gene_choices_prompt(self, prompt, num_genes_pick, remaining_genes):
    ...
    def process_valid_output(self, gene_next_sample, curr_sample, gene_sampled,
                             dropped_genes, args):
